...
Inspect existing artifacts first and resume at the earliest incomplete stage. Preserve completed work and use the scripts' resumable behavior. Stop early only when the user limited the requested scope or progress genuinely requires user action.
...
Use `scripts/seed_openalex.py` to build the initial candidate set and shortlist. First define the evidence mode and competency questions, test the user's wording, then record the field's core phrase and combine it with focused strand operators. Use `content-qualified` access for a bounded, immediately downloadable exploratory vault and disclose its access bias; use `comprehensive` discovery whenever evidential eligibility must not depend on access. Let the agent judge terminology, relevance, strands, anchors, source roles, counterevidence, concentration, and stopping; use the script for requests, logging, candidate merging, access routes, and version deduplication. Do not combine citation count, venue, recency, relevance, and access into one quality score. Aim for 80–100 retained sources only when that covers the competency questions. Do not download papers or create source notes during seeding. `state/candidates.json` is working state, `state/search-log.jsonl` is the audit trail, `state/shortlist.json` is the acquisition set, and `state/access-gaps.json` records selected evidence that lacks a current content route.
